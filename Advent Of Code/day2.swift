...
func day2() {
	...
	let passwords: [Password] = input.components(separatedBy: "\n").map({line in
		let parts = line.components(separatedBy: ": ")
		let parts_two = parts[0].components(separatedBy: " ")
		let parts_three = parts_two[0].trimmingCharacters(in: .whitespacesAndNewlines).components(separatedBy: "-")
		return Password(min: Int(parts_three[0])!, max: Int(parts_three[1])!, char: parts_two[1], password: parts[1])
	})
	
	/// MARK : Count number of passwords which contains at minimum a and maximum b occurences of x
	func part1() -> Int {
		var valid_passwords = 0
		for password in passwords {
			let count: Int = password.password.components(separatedBy: password.char).count - 1
			if password.password.contains(password.char) && count >= password.min && count <= password.max {
				valid_passwords += 1
			}
		}
	...
	/// MARK : Count number of passwords where character at position a and b is x
	func part2() -> Int {
		var valid_passwords = 0
		for password in passwords {
			let char_a: String = String(password.password[password.password.index(password.password.startIndex, offsetBy: password.min - 1)])
			let char_b: String = String(password.password[password.password.index(password.password.startIndex, offsetBy: password.max - 1)])
			if (char_a == password.char && char_b != password.char) || (char_a != password.char && char_b == password.char) {
				valid_passwords += 1
			}
		}
