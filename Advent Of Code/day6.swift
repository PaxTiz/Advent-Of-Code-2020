...
func day6() {
	...
	var groups: [[String]] = [[]]
	for item in input.trimmingCharacters(in: .whitespacesAndNewlines).components(separatedBy: "\n") {
		if item == "" {
			groups.append([])
		} else {
	...
	func part1() -> Int {
		...
		for group in groups {
			...
			for item in group {
				for letter in item {
					if questions[String(letter)] == nil {
						questions[String(letter)] = 1
					} else {
						questions[String(letter)]! += 1
					}
				}
	...
	func part2() -> Int {
		...
		for group in groups {
			...
			sum += questions.filter { (k, v) in v == group.count }.count
		}
